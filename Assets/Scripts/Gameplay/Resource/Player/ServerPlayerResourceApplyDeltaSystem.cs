...
namespace AnimarsCatcher.Gameplay
{
    ...
    public partial struct ServerPlayerResourceApplyDeltaSystem : ISystem
    {
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var buffer in SystemAPI.Query<DynamicBuffer<FoodResourceDeltaEvent>>())
            {
                for (int i = 0; i < buffer.Length; i++)
                {
                    ...
                    resourceState.FoodAmount += eventData.Amount;
            ...
            foreach (var buffer in SystemAPI.Query<DynamicBuffer<CrystalResourceDeltaEvent>>())
            {
                for (int i = 0; i < buffer.Length; i++)
                {
                    ...
                    var resourceState = resourceStates[idx];
                    resourceState.CrystalAmount += eventData.Amount;
                    resourceStates[idx] = resourceState;
                }
