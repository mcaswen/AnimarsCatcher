...
### 6A.5.1 100000 Ani 扩展压力实验

100000 Ani 已作为阶段六预算之外的实验档位接入统一 Harness。开放场景使用 `384×384` 网格，障碍场景使用 `192×192` 网格与 `2 m` Cell；两类网格都覆盖完整出生范围和目标区域。实验仍采集 720 个固定 Server Tick，额外收尾 Tick 只用于等待分时任务与移动完成，不进入性能样本；报告中的 `PerformanceGateEligible` 保持为 `false`，不能代替 10000 Ani 正式门禁。

- 开放场景报告 `GridNavigation_100000_Open_20260829_174340.json`：100000/100000 到达，Field 构建数为 0，Server Tick P50/P95/P99 为 `15.7827 / 17.0272 / 18.2431 ms`，Worker P95 为 `13.8808 ms`，Native 内存为 `174.19 MiB`
- 高复用场景报告 `GridNavigation_100000_ObstacleHighReuse_20260829_175407.json`：100000/100000 到达，4 个目标 Record、655 次共享命中，Server Tick P50/P95/P99 为 `12.7568 / 13.4337 / 341.6457 ms`，Worker P95 为 `10.3790 ms`，Native 内存为 `42.78 MiB`
- 低复用场景报告 `GridNavigation_100000_ObstacleLowReuse_20260829_175721.json`：100000/100000 到达，32 个目标 Record、733 次共享命中，Server Tick P50/P95/P99 为 `15.9054 / 17.8334 / 292.3475 ms`，Worker P95 为 `14.6240 ms`，Native 内存为 `37.10 MiB`
- 三类场景采样期托管分配 P95 均为 `0 B`，没有路径失败、请求排队超时或 Native 容量异常
- 开放场景和低复用场景的 P95 已超过 `16.667 ms`，三类场景的 Worker P95 都超过 `8 ms`；障碍场景 P99 达到数百毫秒，因此当前实现只能证明 100000 Ani 可以完成移动，不能证明能够稳定运行在 60 Hz
- 最大尖峰分别为 `17.081 s`、`8.172 s` 和 `19.123 s`；分段数据表明主要问题来自首次 Cohort 拆分，以及障碍场景的目标场构建与发布，后续扩展必须先把这些工作改成有预算的跨 Tick 增量流程
