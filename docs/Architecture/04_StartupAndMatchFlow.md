...
- `SCN_MainMenu` 是联机入口。账号、创建房间、加入房间和 LAN 房间发现都从这里开始，同时创建跨场景保留的 `EventBus` 与 `GlobalLoadingUI`
...
sequenceDiagram
    ...
    Menu->>HostUI: GameRoomCreatedEventData
    HostUI->>Server: NetCodeServerController.StartListen(7979)
    HostUI->>HostClient: NetCodeClientConnector.RequestConnect(loopback)
...
NetCode 为连接分配 `NetworkId` 后，客户端通过 `ClientLobbyIntroRpc` 上报玩家名称。服务器再把成员加入事件转给房主 UI。
...
sequenceDiagram
    ...
    HostUI->>HostClient: HostStartGameHelper.SendStartGameRpc
    HostClient->>Server: StartGameRpc(sceneName)
...
sequenceDiagram
    participant Client
    participant SetInGame as ServerSetInGameRpcSystem
    ...
    Client->>SetInGame: SetInGameRpc
    SetInGame->>Conn: 读取 SourceConnection.NetworkId
    SetInGame->>Policy: GetCampForConnection
    SetInGame->>SetInGame: 选择对应 Camp 出生点
    SetInGame->>Spawn: InstantiateAndInitialize
    Spawn->>Ghost: LocalTransform + GhostOwner + Camp
    SetInGame->>Conn: NetworkStreamInGame + PlayerSpawnedTag
    SetInGame->>Conn: CommandTarget = Ghost
    Ghost-->>Client: Ghost Snapshot
```

服务器收到 `SetInGameRpc` 后，以 RPC 自带的源连接为身份依据，不接受客户端自行声明玩家编号。它先通过 `ServerCampAssignmentPolicy` 确定阵营，再选择对应出生点并创建玩家角色 Ghost。
...
`EnsureClientCommandTargetSystem` 在客户端找到本地拥有的 owner-predicted 角色，并补齐客户端引用。
...
5. 查找 `MainEntityCamera`，并写入 `ControlledCamera`
...
```text
StartServerListenSystem / StartClientConnectSystem
-> ClientGoInGameSystem
-> GoInGameRequest
-> ServerGoInGameDebugSystem
-> 调试角色出生
```

这条路径绕过主菜单、大厅、`StartGameRpc`、正式场景过渡和 Ready 判定，只用于本地联调。因此，Editor 中能直接进入游戏并不代表正式房间链路也能工作。排查开局故障时应先确认当前使用的是哪一条路径。
