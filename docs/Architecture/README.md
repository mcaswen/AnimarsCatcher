...
01 至 07 描述当前仓库实现，是理解和维护项目的事实基线，不替代 [开发规范](../Standards/DevelopmentGuidelines.md)。08 描述 Grid 移动目标架构，09 记录性能基准方法，10 同时记录阶段计划和实际进度，11 记录程序集迁移方案，12 至 17 记录阶段零至阶段五的实施结果。Grid 烘焙、端点投影和普通 A* 路径服务已经实现，HPA*、阵型、避碰和正式后端切换仍是后续工作。如果代码与事实文档不一致，应先以实际运行结果为准，再同步修正文档。
...
仓库中目前有 268 个自有业务脚本。Core、Gameplay Contracts、Navigation、Gameplay、Player、Player Editor、Networking 和 Presentation 已进入 8 个自定义程序集，Legacy Benchmark 和少量 Authoring、Editor 代码仍主要编译到 `Assembly-CSharp`。
...
- **模块隔离正在渐进落地**：Core、Gameplay Contracts、Navigation、Gameplay、Player、Networking 和 Presentation 已有 asmdef 编译边界，Legacy Benchmark 和剩余 Editor、Authoring 仍在后续迁移范围
