...
2. 公共类型、公共方法和受保护方法使用 XML 文档注释；网络协议、状态机等公开契约必须说明用途和约束。
3. 公共字段、公共属性和公共常量不使用 XML 文档注释；名称不足以表达数据语义、单位、生命周期或约束时，在声明前使用简短中文 `//`，自解释成员可以不写注释。
4. 公共类中的私有成员和默认私有成员不使用 XML 文档注释；自解释实现不写注释，确需解释原因、约束或非直观行为时使用中文 `//` 注释。
5. 删除 AI 生成残留的“第一步、第二步、新增代码、引用命名空间”等无上下文价值注释。
6. 临时调试日志、注释掉的大段旧实现和无效 `using` 不进入正式提交。
7. `#pragma warning disable` 必须限定范围并说明原因。
8. 注释正文统一使用中文；API、类型名、参数名和业内通用缩写可以保留英文，但完整语义必须由中文表达。
9. 注释正文结尾不使用中文句号 `。` 或英文句号 `.`；XML 标签独占一行时不受此限制。
10. 中文注释使用正常中文标点分隔分句和并列项，按语义使用 `，`、`、`、`；`、`：` 等符号，不使用空格代替标点；API、类型名和参数名两侧可以保留必要空格。
11. 需要 XML 文档的类型或方法即使说明只有一行，`<summary>`、正文和 `</summary>` 也必须各占一行，禁止写成 `/// <summary>说明</summary>`。
12. 寻路、空间划分、几何计算、数值计算、状态同步等复杂算法必须提供阶段级注释，帮助维护者理解算法模型和正确性依据。
13. 算法阶段注释直接描述当前代码的意图，例如“投影请求端点”“过滤 Clearance 不足的 Portal”“回滚未完成切片”；禁止使用“阶段一、阶段二、第一步、第二步”等脱离代码语义的编号标题。
14. 函数体内的流程注释必须紧邻其解释的判断、循环、计算、状态切换、写回或资源操作；不得在函数入口集中堆放随后才会分别发生的多段流程说明。
15. 一条函数体注释只解释紧随其后的一个逻辑点。多个约束分别作用于不同语句时，应将说明分散到对应语句前；只有适用于整个函数的算法模型或全局不变量才放在函数入口。
16. 函数体注释通常保持一至两行，优先解释“为什么这样判断”“该语句维持什么不变量”“失败时为何必须回滚”，不复述变量名、循环动作和赋值结果。
17. 复杂算法注释按实际风险说明输入数据的语义、选择该算法的原因、关键不变量或公式、边界条件、失败路径、保守性取舍与时间或空间复杂度，不要求机械覆盖每一项。
18. 注释应放在算法阶段、非直观公式和关键分支之前，解释“为什么成立”和“修改时不能破坏什么”，不逐行复述循环、赋值和条件判断。
19. 算法参考论文、文章或开源实现时，应记录可长期访问的来源，并说明项目内的关键改动和适用边界；禁止复制大段原文充当注释。
20. 修改算法模型、数据含义或边界策略时，必须同步更新相关注释、测试和架构文档，过时注释按缺陷处理。
21. `Baker.Bake`、`ISystem.OnCreate`、`ISystem.OnUpdate`、`ISystem.OnDestroy`、MonoBehaviour 生命周期函数、Editor 回调以及简单的接口实现或重写方法属于框架模板入口；当方法职责和实现都直接明确时，不要求添加 XML 文档或普通注释，也不为重复说明模板职责而补注释。
22. 模板入口包含复杂生命周期、System 排序、World 边界、资源所有权、异步状态、失败回滚或其他非直观副作用时，只在相关阶段和关键分支前使用中文注释解释约束；方法级说明仅在入口本身形成公共契约且无法从类型和签名直接理解时添加。
...
```csharp
// Hash 只用于初筛，命中前仍需比较完整 Corridor
if (entry.CorridorHash != corridorHash)
{
    continue;
}

// 反向松弛仍按 predecessor 到 current 的正向移动成本计算
float candidateCost = costs[current] + CalculateStepCost(predecessor, current);
```

禁止把多个尚未发生的流程集中写在函数入口：
...
1. 项目自有业务 C# 源码的注释率必须大于等于 **12%**。
2. 推荐将模块注释率维持在 **17% 左右**，合理区间为 16% 至 18%。
3. 注释率按模块或本次提交涉及的业务源码整体统计，不要求每个数据组件和每个短文件单独达到 12%。
4. 默认计算方式：
