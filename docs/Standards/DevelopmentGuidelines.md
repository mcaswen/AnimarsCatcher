...
项目自有业务 C# 源码注释率必须大于等于 **12%**，推荐维持在 **17% 左右**。注释正文统一使用中文，API 和类型名等必要技术名称可以保留英文；中文分句和并列项使用正常中文标点，不以空格代替标点，正文结尾不使用中文句号或英文句号。公共类型和公共方法使用分行书写的 XML 文档注释；公共字段、属性和常量不使用 `<summary>`，确需说明语义或约束时使用简短中文 `//`。复杂算法还必须说明算法模型、关键不变量、边界条件和重要性能取舍；函数体流程注释直接写当前逻辑意图并紧邻对应语句，不使用“阶段一、阶段二”等编号标题，也不在函数入口堆放后续流程说明。详细计算范围、例外和防止无意义注释的要求见 [代码、DOTS 与 NetCode 规范](02_CodeArchitecture.md#42-注释率)。
