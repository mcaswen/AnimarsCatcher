...
- 客户端将框选结果排序后分块提交，服务器按版本、Hash 和所有权发布权威选择集；`AniCommandRpc` 只引用已确认版本，再由当前后端接入 FSM 或 Grid 移动链路
...
- Grid Stage 6A.0～6A.4 已完成功能实现：统一基准支持 512～10000 Ani 规模输入，正式指令支持万人选择集、有界导航分组、自然目标区域、共享流向场调度和并行单位移动；万人完整回放尚未通过性能门禁

仍在推进：

- Grid 正式场景的窄门、连续窄道和动态障碍退出验收
- 先完成 6A.5 目标流向场共享与性能修复，通过万人性能门禁后再推进 6B.1 空间哈希、ORCA 局部避碰与世界碰撞
