...
namespace Unity.NetCode.Tests
{
    internal static class GhostGenTestUtils
    {
        ...
        internal struct GhostGenTypesClamp_Values
        {
            ...
            [GhostField] public NetworkEndpoint EndpointInvalid;
            [GhostField] public NetworkEndpoint EndpointIPv4;
            [GhostField] public NetworkEndpoint EndpointIPv6;
        ...
        public static void VerifyGhostValuesClamp_Values(bool hasPartialSupport, GhostGenTypesClamp_Values serverValues, GhostGenTypesClamp_Values clientValues, Entity serverGhost, Entity clientGhost)
        {
            ...
            Assert.IsTrue(serverValues.EndpointInvalid.Equals(clientValues.EndpointInvalid), $"Endpoints are not equal: client {clientValues.EndpointInvalid} server {serverValues.EndpointInvalid}");
            Assert.IsTrue(serverValues.EndpointIPv4.Equals(clientValues.EndpointIPv4), $"Endpoints are not equal: client {clientValues.EndpointIPv4} server {serverValues.EndpointIPv4}");
            Assert.IsTrue(serverValues.EndpointIPv6.Equals(clientValues.EndpointIPv6), $"Endpoints are not equal: client {clientValues.EndpointIPv6} server {serverValues.EndpointIPv6}");
            Assert.IsTrue(serverValues.EndpointCustom.Equals(clientValues.EndpointCustom), $"Endpoints are not equal: client {clientValues.EndpointCustom} server {serverValues.EndpointCustom}");
        }
...
",
        ...
        public static GhostGenTypesClamp_Values CreateGhostValuesClamp_Values(int baseValue, Entity ghostEntity)
        {
            ...
            var values = new GhostGenTypesClamp_Values()
            {
                ...
                EndpointInvalid = GetEndpointToTest(NetworkFamily.Invalid),
                EndpointIPv4 = GetEndpointToTest(NetworkFamily.Ipv4),
        ...
        public static NetworkEndpoint GetEndpointToTest(NetworkFamily family)
        {
            string adr = "";
            ushort port = 1234;
            NetworkEndpoint toTest;
            switch (family)
            {
                ...
                case NetworkFamily.Custom:
                    toTest = new NetworkEndpoint();
                    toTest.SetRawAddressBytes(new NativeArray<byte>(new byte[]{1, 2, 3, 4, 5}, Allocator.Temp), family);
                    break;
                case NetworkFamily.Invalid:
                    toTest = new NetworkEndpoint();
                    toTest.Family = NetworkFamily.Invalid;
                    break;
                default:
                    throw new NotImplementedException( $"NetworkFamily {family} is not handled by this test");
            }
